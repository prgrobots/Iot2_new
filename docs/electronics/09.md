...
!!! tip "Setup Check"
    - [ ] All Pico W boards programmed and tested individually
...
!!! info "Pico W: MPU6050 on a separate breakout board for the truck"
    Assessment 3 used the **MPU6050 soldered to the Pico W carrier board** (via the on-board I²C connection). In this session everything consolidates onto a single **Pico W** running the Arduino framework — the same board that will publish to AWS IoT Core.

    **What this means practically:**

    - You need a **separate GY-521 breakout board** wired to the Pico W via I²C. The MPU6050 on your carrier board is still there but is used through the same I²C bus.
    - The filtering algorithm you wrote in Session 8 carries over unchanged — same logic, same Arduino C++ syntax.
