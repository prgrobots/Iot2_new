# Session 17: AWS Industry Quest – Manufacturing & Automotive (Week 3/3)

**Week:** 17  
**Element:** ICTIOT503 Element 3: Deploy complete IoT solution  
**Duration:** ~4–5 hours  
**Phase:** Capstone Integration (AWS Industry Quest: Manufacturing & Automotive – Final)

---

!!! info "Week 17 Build Activity"
    This AWS session runs **in parallel** with fault scenario testing. While completing these labs, you should also be working through the [Week 17 Testing Scenarios guide](../capstone/testing.md) — that page covers the 3 required fault scenarios, test report, performance data, and Week 17 deliverables. The predictive maintenance concepts from Labs 7-13 feed directly into your CloudWatch alarm and SNS alert evidence.

## Session Introduction

**Week 17** is the final week of the **AWS Industry Quest: Manufacturing & Automotive**. You'll complete Labs 7–13 of the quest, covering the full manufacturing scenario, advanced vehicle telemetry, multi-truck predictive maintenance, and prepare your complete Capstone submission.

**By the end of Week 17:** You will have a **fully integrated IoT system** combining:
- Haul truck (Pico W) with sensors
- AWS IoT Core + Device Shadows
- SiteWise asset models
- QuickSight dashboard (Week 14)
- Digital twins in TwinMaker (Weeks 15–16)
...
- Demonstrate full end-to-end IoT system (truck → cloud → analytics → pit station)
...
1. **AWS Industry Quest Labs 7–13** (final labs of the 13-hour quest)
   - Lab 7: Manufacturing scenario completeness
   - Lab 8–10: Vehicle telemetry deep dives
   - Lab 11–13: Multi-truck simulation and scaling

2. **Capstone Integration** – Bring everything together
3. **Documentation** – Portfolio mapping and technical architecture
4. **Video Preparation** – 5-minute system walkthrough
...
!!! info "Completed by End of Week 16"
    - AWS Industry Quest Labs 1–6 complete
...
!!! tip "System Health Check"
    - [ ] Haul truck (Pico W) publishing continuous MQTT telemetry
    ...
    - [ ] GitHub repository with all code from Weeks 1–16
...
## AWS Industry Quest Labs 7–13: Manufacturing Mastery

### Labs 7–10: Vehicle Telemetry & Manufacturing Context
**Combined Duration:** 2–2.5 hours
...
1. **Expand SiteWise Asset Model** for your truck:
   - Properties:
     - `engine_temperature` (from thermistor)
     - `vibration_level` (from GY-521)
     - `cabin_lock_state` (from RFID)
...
3. **Implement Cross-Device Rules:**
   - If any truck's vibration anomaly detected → trigger pit station alarm
   - Aggregate metrics across fleet (in Week 18, imagine 20 trucks)

### Labs 11–13: Multi-Truck Simulation & Scaling
**Duration:** 1.5–2 hours

**Concepts:**
- Scaling IoT workloads to fleet (1 → 20 trucks)
...
2. **Pit Station LCD Simulation** (final software piece):
   - Create a simple **Node.js/Python web dashboard** or **HTML page** showing:
     ...
     - Click truck → drill down to detailed metrics
   ...
   **Minimal Example (HTML):**
   ...
   <body>
       <h1>RockCore Mining – Pit Station Dashboard</h1>
       ...
       <script>
           ...
           trucks.forEach(truck => {
               ...
               const badge = truck.live ? '🟢 LIVE' : '⚪ Simulated';
               grid.innerHTML += `
...
```
Physical Truck → MQTT → IoT Core → {Rules, Shadows, Alerts}
                                  ↓
...
### 2. Technical Specifications (1–2 pages)
...
- [ ] `arduino/haul-truck-iot.ino` – Full truck firmware
- [ ] `aws/iot-rules.json` – All IoT Core rules
- [ ] `aws/lambda-twinmaker-update.py` – TwinMaker updater
- [ ] `dashboard/pit-station.html` – Dashboard HTML/CSS/JS
- [ ] `docs/ARCHITECTURE.md` – System diagram + explanation
- [ ] `docs/UNIT_MAPPING.md` – Performance criteria alignment

### Video (5 minutes) – Prepared for Week 18 Submission
**Script:**
1. **Introduction (30 s):** "RockCore IoT System – Integrated Fleet Monitoring"
...
A: **Labs 1–6 are required** — they cover TwinMaker modeling, data binding, and fleet simulation, which are all explicitly assessed in A6. **Labs 7–13 are strongly recommended** but do not have their own separate deliverable; they deepen the predictive maintenance understanding behind your CloudWatch alarm evidence. If time is short, complete Labs 1–6 in full first, then do as much of 7–13 as time allows.

**Q: Can I submit Capstone without real truck hardware?**  
A: No. A6 requires at least one real Pico W publishing live telemetry. You can simulate other trucks, but the "real" truck is mandatory.

**Q: How much time should I spend on pit station web dashboard?**  
A: Minimal viable: 30 minutes (basic HTML grid). Enhanced: 1–2 hours (add charts, drill-down, real-time WebSocket updates). Aim for MVP by end of Week 17.
...
[← Week 16 (Industry Quest 2/3)](16.md) | [Course Overview](../index.md) | [Testing Scenarios →](../capstone/testing.md)
